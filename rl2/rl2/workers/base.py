...
class RolloutWorker:
    ...
    def rollout(self):
        ...
        self.ep_steps = self.ep_steps + np.ones_like(done, np.int64)
    ...
    def run(self):
        for step in range(self.max_steps // self.n_env + 1):
            ...
            if self.render:
                ...
                # cond = done if np.asarray(done).size == 1 else any(done)
                if cond:
                    if self.time_to_log_image and not self.start_log_image:
                        self.time_to_log_image = False
                ...
                if self.start_log_image:
                    ...
                    self.logger.video_summary(tag="playback", step=self.num_steps)
                    self.store_image = False
            ...
            if self.num_steps % self.log_interval < self.n_env:
                ...
                info_r = {
                    ...
                    # 'Loss/All': info['Loss/All']
                    # 'Score/score': np.mean(list(self.scores))
                }
    ...
    def run(self):
        while self.num_episodes < self.max_episodes:
            ...
            if self.render and self.start_log_image:
                image = self.env.render(self.render_mode)
                ascii_mode = self.env.render(mode="ascii")
                self.map.append(ascii_mode)
                self.logger.store_rgb(image)
            ...
            if log_cond:
                if self.start_log_image:
                    print("save video")
                    # with open("map.csv", "a", newline="", encoding="utf-8") as file:
