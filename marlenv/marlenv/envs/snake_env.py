...
class SnakeEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        # for snake in self.snakes:
        #     self.rows[i].append([
        #         self.reward_dict['fruit'] * snake.fruit,
        #         self.reward_dict['kill'] * snake.kills,
        #         self.reward_dict['time'] * snake.alive
        #     ])
        #     i += 1
        ...
        dones = []
        fruits, kills = [], []
        # Update snake rews, stats, etc., and update the grid accordingly1
        i = 0
        for snake in self.snakes:
            if not snake.death and not snake.alive:
                ...
                snake.reward += (self.reward_dict['lose'] / max(1, self.epi_steps[i])) * snake.death
                ...
                self.rows[i].append([
                    ...
                    1 if actions[i] == 2 else 0,
                    1 if snake.fruit > 0 else 0,
                    1 if snake.kills > 0 else 0
                    # self.reward_dict['fruit'] * snake.fruit,
                    # self.reward_dict['kill'] * snake.kills
                    # self.reward_dict['time'] * snake.alive
                ])
    ...
    def save_rows(self, data_dir):
        for i in range(self.num_snakes):
            with open(data_dir+'_agent'+str(i)+'.csv', 'a', newline='', encoding='utf-8') as file:
